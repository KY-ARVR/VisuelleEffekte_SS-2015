...
//     page 99      (physics)
//     page 143-144 (implementation)


precision highp float;


struct MinnaertParameters
{
  vec4  diffuseColor;
  float roughness;
};
...
uniform MinnaertParameters minnaertParameters;
...
void main (void)
{
  ...
  diffuseColor = minnaertParameters.diffuseColor;
  ...
  if (texturing.enabled == 1)
  {
    ...
    if (texturing.mode == 1)
    {
      diffuseColor = diffuseColor * minnaertParameters.diffuseColor;
    }
  ...
  for (int lightIndex = 0; lightIndex < NUMBER_OF_LIGHTS; lightIndex++)
  {
    ...
    diffuseTerm = pow (dotOfNormalAndEye * dotOfNormalAndLight,
